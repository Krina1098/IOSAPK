...
@WebServlet("/AdminPassUpdate")
public class AdminPassUpdate extends HttpServlet{
	public void doMethod(HttpServletRequest request,HttpServletResponse response)
	{
		...
		if(pass.equals(conf))
		{
			int row;
			try {
				stmt=conn.prepareStatement("Update ADMINCREDENTIAL set PASS=? where HINT=? AND USERNAME=?");
				...
				{
					RequestDispatcher rd= request.getRequestDispatcher("AdminPasswordReset.html");
					try {
						response.getWriter().println("<!DOCTYPE html>\r\n" + 
						 		"<html lang=\"en\" dir=\"ltr\">\r\n" + 
						 		"  <head>\r\n" + 
