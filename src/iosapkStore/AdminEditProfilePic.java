...
@WebServlet("/AdminEditProfilePic")
@MultipartConfig

public class AdminEditProfilePic extends HttpServlet{
		...
		public void doMethod(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException, SQLException
		{
			...
			InputStream img=null;
		     ...
		     int row;
		     stmt=conn.prepareStatement("Update ADMINCREDENTIAL set IMAGE=? where USERNAME=?");
		     stmt.setBlob(1, img);
				...
				if(row>0)
				{
					...
					try {
						response.getWriter().println("<!DOCTYPE html>\r\n" + 
						 		...
						 		"      alert(\"Admin Profile Pic Updated Successfully!!!\");\r\n" + 
						 		"    </script>\r\n" + 
						 		"\r\n" + 
						 		"  </body>\r\n" + 
						 		"</html>");
